...
export function ProjectPicker({
  ...
  return (
    <nav className="project-picker" aria-label="Projects">
      <button
        aria-current={selected === "B04" ? "page" : undefined}
        onClick={() => onSelect("B04")}
      >
        <b>B04</b> Episode Desk
      </button>
      {products.map((p) => (
        <button
          key={p.code}
          aria-current={selected === p.code ? "page" : undefined}
...
export default function Products({ code }: { code: string }) {
  ...
  return (
    <main className="products-shell">
      <div className="product-heading">
        <div>
          <span className="eyebrow">OVERSEAS STUDIO / {code}</span>
            ...
            <>
              <section className="product-panel">
                ...
                <div className="product-tools">
                  ...
                  {code === "B06" && (
                    <a
                      href={`/api/public/sites/${record.id}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      Open published site ↗
