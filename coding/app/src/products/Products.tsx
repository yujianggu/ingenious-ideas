...
export function ProjectPicker({
  ...
  return (
    <View style={s.card}>
      ...
      <View style={s.row}>
        {[{ code: "B04", name: "Episode Desk" }, ...products].map((p) => (
          <Button
            key={p.code}
            quiet={selected !== p.code}
            title={`${p.code} · ${p.name}`}
...
export default function Products({
  ...
  const consumer = ["C14", "C15", "C17"].includes(code);
  ...
  async function toggle(item: any, current: CurrentWork) {
    requireCurrent(current);
    if (!record) return;
    if (offline || snapshotRef.current?.pending.length) {
      const cached = snapshotRef.current;
      if (!cached)
        throw new Error(
          "Save this list for offline use while connected first.",
        );
      const next = queueCheck(cached, record.id, item.id, !item.checked);
      await persist(next, current);
      requireCurrent(current);
      setRecords(next.records);
      setRecord(next.records.find((r) => r.id === record.id) || null);
      setNotice("Check saved on this device. Sync when connected.");
      return;
    }
    try {
      await mutate(
      ...
      setRecord(next.records.find((r) => r.id === record.id) || null);
      setError("");
      setNotice("Offline check queued. Sync when connected.");
    }
  }
  async function sync(current: CurrentWork) {
    ...
    await syncPendingChecks(
      ...
      {
        get: (recordId) =>
          request<ProductRecord>(base, token, `${path}/${recordId}`),
        post: (command, server) =>
          request<ProductRecord>(
            base,
            token,
            `${path}/${command.recordId}/actions/toggle-item`,
            "POST",
            {
        ...
        persist: (value) => persist(value, current),
        progress: (value) => {
          setRecords(value.records);
          if (record)
            setRecord(value.records.find((r) => r.id === record.id) || null);
        },
      },
      current,
    );
    requireCurrent(current);
    setOffline(false);
    setNotice("All local checks synced.");
  }
    ...
    <View>
          ...
          <View>
            ...
            <View style={s.row}>
              ...
              {!!snapshot?.pending.length && (
                <>
                  <Button
                    title={`Sync ${snapshot.pending.length} local checks`}
                    disabled={busy}
                    onPress={() => void run(sync)}
                  />
                  <Button
                    quiet
                    title="Review cloud and discard local checks"
                    disabled={busy}
                    onPress={() =>
                      void run(async (current) => {
                        const latest = await fetchRecords(current);
                        const changes = snapshotRef.current?.pending || [];
                        const details = changes
                          .map((c) => {
                            const cloud = latest.find(
                              (r) => r.id === c.recordId,
                            );
                            const item = cloud?.items?.find(
                              (i: any) => i.id === c.itemId,
                            );
                            const local = snapshotRef.current?.records
                              .find((r) => r.id === c.recordId)
                              ?.items.find((i: any) => i.id === c.itemId);
                            return `${local?.label || "Item"}: local ${c.checked ? "packed" : "unpacked"}, cloud ${item ? (item.checked ? "packed" : "unpacked") : "missing"}`;
                          })
                          .join("\n");
                        if (
                          !(await confirm(
                            `${details}\nDiscard the listed local checks and keep cloud data?`,
                          ))
                        )
                          return;
                        requireCurrent(current);
                        ...
                        if (record)
                          setRecord(
                            latest.find((r) => r.id === record.id) || null,
                          );
                        setOffline(false);
                        setEpoch((n) => n + 1);
                      })
      ...
      {creating ? (
        ...
        <>
          ...
          {code === "C15" && record.items?.length > 0 && (
            <View style={s.card}>
              <Text style={s.heading}>Packing checklist</Text>
              {record.items.map((item: any) => (
                <View key={item.id} style={s.checkRow}>
                  <Switch
                    ...
                    disabled={busy}
                    onValueChange={() =>
                      void run((current) => toggle(item, current))
                    }
                  />
          ...
          {code === "C14" && record.attachments?.length > 0 && (
            <View style={s.card}>
              <Text style={s.heading}>Original documents</Text>
