...
def create_app(data_dir=None):
    app=FastAPI(title='B09 · 销售研究与邮件草稿'); s=Store(data_dir or os.getenv('B09_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('B09_CORS_ORIGINS','http://localhost:5119,http://localhost:8119,http://127.0.0.1:5119,http://127.0.0.1:8119').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
