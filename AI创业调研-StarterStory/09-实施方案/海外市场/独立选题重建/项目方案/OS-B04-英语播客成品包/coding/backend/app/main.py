...
def create_app(data_dir=None):
    app=FastAPI(title='Episode Desk'); s=Store(data_dir or os.getenv('EPISODE_DATA_DIR','data')); app.state.store=s
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('EPISODE_CORS_ORIGINS','http://localhost:5173,http://localhost:8081,http://127.0.0.1:5173,http://127.0.0.1:8081').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
    ...
    async def limits(request:Request,call_next):
        ...
        maximum=int(os.getenv('EPISODE_MAX_UPLOAD_BYTES',str(100*1024*1024)))+1024*1024 if '/assets' in request.url.path else 1024*1024
