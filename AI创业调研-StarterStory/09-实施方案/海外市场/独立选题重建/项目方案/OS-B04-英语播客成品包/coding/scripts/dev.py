...
def main():
    ...
    commands = [
        (ROOT / 'backend', [str(python), '-m', 'uvicorn', 'app.main:app', '--host', os.getenv('API_HOST', '127.0.0.1'), '--port', '8000']),
        (ROOT / 'frontend', npm() + ['run', 'dev', '--', '--host', '127.0.0.1', '--port', '5173']),
    ]
    children = []
    stopping = False
    def stop(*_):
    ...
    try:
        ...
        print('Web: http://127.0.0.1:5173 | API: http://127.0.0.1:8000/docs', flush=True)
        while not stopping:
