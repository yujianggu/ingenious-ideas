...
网页 http://127.0.0.1:5173，API http://127.0.0.1:8000/docs。首次注册本项目账号。其他 idea 的账号与会话不会自动共享。

移动端另开终端进入 `app`，执行 `npm start -- --port 8081`，使用开发客户端/Expo 支持的运行环境；浏览器预览 `npm run web -- --port 8081`。真机在 Connection 中填写局域网 API 地址，并用 `API_HOST=0.0.0.0 python3 scripts/dev.py` 启动后端。
...
默认保存在本项目 `backend/data`，可用 `EPISODE_DATA_DIR` 指向独立目录。CORS 使用 `EPISODE_CORS_ORIGINS`。不要将数据库、会话、密码或本地环境文件提交到 Git。
