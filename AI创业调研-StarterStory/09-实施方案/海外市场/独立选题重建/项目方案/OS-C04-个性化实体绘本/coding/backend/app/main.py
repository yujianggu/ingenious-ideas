...
def create_app(data_dir=None):
    app=FastAPI(title='C04 · 个性化实体绘本'); s=Store(data_dir or os.getenv('C04_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('C04_CORS_ORIGINS','http://localhost:5124,http://localhost:8124,http://127.0.0.1:5124,http://127.0.0.1:8124').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
