...
it("surfaces stale revision as recoverable conflict with server explanation", async () => {
  ...
  await expect(request("/products/C04/records/one")).rejects.toMatchObject({
    status: 409,
...
it("normalizes validation arrays into readable errors", async () => {
  ...
  await expect(request("/products/C04/records")).rejects.toThrow(
    "duration: Must be at least 90",
  );
});
it("expires the session on unauthorized response", async () => {
  sessionStorage.setItem("os-c04-token", "expired");
  vi.stubGlobal(
    "fetch",
    async () =>
  ...
  await expect(request("/products/C04/records")).rejects.toBeInstanceOf(
    ApiError,
  );
  expect(sessionStorage.getItem("os-c04-token")).toBeNull();
});
