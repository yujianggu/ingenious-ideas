...
it("surfaces stale revision as recoverable conflict with server explanation", async () => {
  ...
  await expect(request("/products/C17/records/one")).rejects.toMatchObject({
    status: 409,
...
it("normalizes validation arrays into readable errors", async () => {
  ...
  await expect(request("/products/C17/records")).rejects.toThrow(
    "duration: Must be at least 90",
  );
});
it("expires the session on unauthorized response", async () => {
  sessionStorage.setItem("os-c17-token", "expired");
  vi.stubGlobal(
    "fetch",
    async () =>
  ...
  await expect(request("/products/C17/records")).rejects.toBeInstanceOf(
    ApiError,
  );
  expect(sessionStorage.getItem("os-c17-token")).toBeNull();
});
