...
def test_product_isolation_revision_export_delete(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, 'app.products.c17', module)
    client = TestClient(create_app(tmp_path))
    alice = session(client, 'alice@example.com'); bob = session(client, 'bob@example.com')
    path = '/api/products/C17/records'
    ...
    assert client.get(url.replace('C17', 'B09'), headers=alice).status_code == 404
...
def test_gateway_protects_envelope(tmp_path, monkeypatch):
    ...
    path = '/api/products/C17/records'; record = client.post(path, json={}, headers=auth).json()
    assert record['id'] != 'injected' and record['code'] == 'C17' and record['revision'] == 1
    url = path+'/'+record['id']
    newer = client.post(url+'/actions/edit', json={'revision': 1}, headers=auth).json()
    assert newer['id'] == record['id'] and newer['code'] == 'C17' and newer['revision'] == 2
