...
def create_app(data_dir=None):
    app=FastAPI(title='C17 · 混合书刊目录'); s=Store(data_dir or os.getenv('C17_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('C17_CORS_ORIGINS','http://localhost:5137,http://localhost:8137,http://127.0.0.1:5137,http://127.0.0.1:8137').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
