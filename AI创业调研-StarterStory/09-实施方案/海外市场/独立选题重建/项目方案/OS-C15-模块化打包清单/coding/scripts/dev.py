...
def main():
    ...
    commands = [
        (ROOT / 'backend', [str(python), '-m', 'uvicorn', 'app.main:app', '--host', os.getenv('API_HOST', '127.0.0.1'), '--port', '8035']),
        (ROOT / 'frontend', npm() + ['run', 'dev', '--', '--host', '127.0.0.1', '--port', '5135']),
    ]
    children = []
    stopping = False
    def stop(*_):
    ...
    try:
        ...
        print('Web: http://127.0.0.1:5135 | API: http://127.0.0.1:8035/docs', flush=True)
        while not stopping:
