...
def create_app(data_dir=None):
    app=FastAPI(title='C15 · 模块化打包清单'); s=Store(data_dir or os.getenv('C15_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('C15_CORS_ORIGINS','http://localhost:5135,http://localhost:8135,http://127.0.0.1:5135,http://127.0.0.1:8135').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
