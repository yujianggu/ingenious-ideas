...
def create_app(data_dir=None):
    app=FastAPI(title='C14 · 离线旅行资料夹'); s=Store(data_dir or os.getenv('C14_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('C14_CORS_ORIGINS','http://localhost:5134,http://localhost:8134,http://127.0.0.1:5134,http://127.0.0.1:8134').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
