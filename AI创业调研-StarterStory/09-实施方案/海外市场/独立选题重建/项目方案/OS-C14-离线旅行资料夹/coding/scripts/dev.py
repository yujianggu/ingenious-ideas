...
def main():
    ...
    commands = [
        (ROOT / 'backend', [str(python), '-m', 'uvicorn', 'app.main:app', '--host', os.getenv('API_HOST', '127.0.0.1'), '--port', '8034']),
        (ROOT / 'frontend', npm() + ['run', 'dev', '--', '--host', '127.0.0.1', '--port', '5134']),
    ]
    children = []
    stopping = False
    def stop(*_):
    ...
    try:
        ...
        print('Web: http://127.0.0.1:5134 | API: http://127.0.0.1:8034/docs', flush=True)
        while not stopping:
