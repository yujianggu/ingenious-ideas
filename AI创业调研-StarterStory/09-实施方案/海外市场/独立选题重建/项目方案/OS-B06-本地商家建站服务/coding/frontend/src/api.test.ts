...
it("surfaces stale revision as recoverable conflict with server explanation", async () => {
  ...
  await expect(request("/products/B06/records/one")).rejects.toMatchObject({
    status: 409,
...
it("normalizes validation arrays into readable errors", async () => {
  ...
  await expect(request("/products/B06/records")).rejects.toThrow(
    "duration: Must be at least 90",
  );
});
it("expires the session on unauthorized response", async () => {
  sessionStorage.setItem("os-b06-token", "expired");
  vi.stubGlobal(
    "fetch",
    async () =>
  ...
  await expect(request("/products/B06/records")).rejects.toBeInstanceOf(
    ApiError,
  );
  expect(sessionStorage.getItem("os-b06-token")).toBeNull();
});
