...
import { ApiError, normalizeError, request } from "../src/api.ts";
...
test("HTTP conflict throws a typed error without mutating supplied data", async () => {
  ...
  const s = server.createServer((_req, res) => {
    ...
    res.end('{"detail":"Refresh this record"}');
  });
  await new Promise<void>((resolve) => s.listen(0, "127.0.0.1", resolve));
  try {
    const addr = s.address() as { port: number };
    await assert.rejects(
      request(`http://127.0.0.1:${addr.port}`, "token", "/records/1"),
      (e: any) =>
        e instanceof ApiError &&
        e.status === 409 &&
        e.message === "Refresh this record",
    );
  } finally {
    s.close();
  }
