...
def create_app(data_dir=None):
    app=FastAPI(title='B06 · 本地商家建站服务'); s=Store(data_dir or os.getenv('B06_DATA_DIR',str(Path(__file__).resolve().parents[1]/'data'))); app.state.store=s
    from .products.gateway import install
    install(app, s)
    app.add_middleware(CORSMiddleware,allow_origins=os.getenv('B06_CORS_ORIGINS','http://localhost:5116,http://localhost:8116,http://127.0.0.1:5116,http://127.0.0.1:8116').split(','),allow_methods=['GET','POST','PUT','OPTIONS'],allow_headers=['Authorization','Content-Type'])
