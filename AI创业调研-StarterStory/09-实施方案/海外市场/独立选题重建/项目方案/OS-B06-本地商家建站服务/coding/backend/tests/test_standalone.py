...
CODE = 'B06'
CREATE, ACTION, BODY = ({'title': 'Local business', 'email': 'hello@example.com', 'phone': '+1 212 555 0100', 'tagline': 'A welcoming home', 'about': 'A local cleaning team.', 'areas': 'Brooklyn'}, 'add-service', {'title': 'Cleaning', 'description': 'House cleaning', 'priceNote': 'Request quote'})
...
def test_published_site_inquiry_and_private_followup(tmp_path):
    client = TestClient(create_app(tmp_path))
    auth = login(client, 'business@example.com')
    record = client.post('/api/products/B06/records', json=CREATE, headers=auth).json()
    url = '/api/products/B06/records/'+record['id']
    public = '/api/public/sites/'+record['id']
    assert client.get(public).status_code == 404
    record = client.post(url+'/actions/add-service', json={**BODY,'revision':1}, headers=auth).json()
    published = client.post(url+'/actions/publish', json={'publicConsent':True,'revision':2}, headers=auth)
    assert published.status_code == 200, published.text
    view = client.get(public)
    assert view.status_code == 200
    assert 'frame-ancestors' in view.headers['content-security-policy']
    inquiry = {'consent':True,'serviceId':record['site']['services'][0]['id'],'area':'Brooklyn','name':'Alex Reader','email':'alex@example.com','message':'Please clean our house next week.'}
    assert client.post(public+'/inquiries', json=inquiry).status_code == 200
    assert client.post(public+'/inquiries', data={**inquiry,'consent':'on'}).status_code == 200
    private = client.get(url, headers=auth).json()
    assert private['revision'] == 5 and len(private['inquiries']) == 2
    assert 'alex@example.com' not in client.get(public).text
    followed = client.post(url+'/actions/follow-up', json={'revision':5,'inquiryId':private['inquiries'][0]['id'],'status':'contacted','note':'Called with available times.'}, headers=auth)
    assert followed.status_code == 200, followed.text
    assert followed.json()['inquiries'][0]['status'] == 'contacted'
    assert client.post(url+'/actions/unpublish', json={'revision':6}, headers=auth).status_code == 200
    assert client.get(public).status_code == 404
    assert client.post(public+'/inquiries', json=inquiry).status_code == 404
